...
var tabs = function(spec, my) {
    ...
    oplog.forEach(function(op) {
      if(typeof op.value !== 'undefined') {
        ...
        switch(op.payload.type) {
          ...
          case 'next': {
            var i = value.active + 1;
            if(i < value.tabs.length) {
              value.active = i;
            }
            else {
              value.active = 0;
          ...
          case 'prev': {
            ...
            if(i >= 0) {
              ...
              value.active = value.tabs.length - 1;
            }
            break;
          }
          case 'commit': {
          ...
          case 'close': {
            if(value.tabs.length > 0) {
              if(op.payload.tab_id) {
                value.tabs.forEach(function(t, i) {
                  if(t.tab_id === op.payload.tab_id) {
                    value.tabs.splice(i, 1)[0]
                  }
                });
              }
              else {
                value.tabs.splice(value.active, 1)[0]
              }
  ...
  core_state_handler = function(state) {
    Object.keys(state).forEach(function(tab_id) {
      ...
      if(tab && (tab.url !== url_for_state(state[tab_id]) ||
        ...
        /* TODO(spolu): Check increasing hops to prevent races */
        push_payload({
          type: 'state',
          tab_id: tab_id,
  ...
  // ### action_next
  //
  // Public action to select next tab
    ...
    async.series([
      ...
      function(cb_) {
        breach.module('core').call('tabs_new', { 
          ...
          if(err) {
            if(err.name === 'core_tabs:id_already_exists') {
              my.new_tab.tab_id = '__NEW_TAB_ID__';
              common.log.out('Tab Scavanged: ' + my.new_tab.tab_id);
              return cb_();
            }
            return cb_(err);
          }
          my.new_tab.tab_id = res.id;
          breach.module('core').call('tabs_context_menu_handler', {
            id: res.id,
  ...
  common.method(that, 'action_next', action_next, _super);
  common.method(that, 'action_prev', action_prev, _super);
