...
var tabs = function(spec, my) {
  ...
  var core_tabs_removed_handler;    /* core_tabs_removed_handler(evt); */
  ...
  var exposed_context_menu_add;     /* exposed_context_menu_add(title, action); */
  var exposed_context_menu_remove;  /* exposed_context_menu_remove(title); */
  ...
  // ### core_tabs_removed_handler
  // Handler called when a tab is closed by the user actions
  // ```
  // @evt { id }
  // ```
  core_tabs_removed_handler = function(evt) {
    var tab_id = evt.id;
    push_payload({
      type: 'close',
      tab_id: tab_id,
  ...
  // ### set of basic context menu items that can be extended from other modules
  ...
  /* ### add context menu item from other modules */
  exposed_context_menu_add = function(src, args) {
    var title = args.title,
      module = args.module,
      action = args.action;

    if (!title || !module || !action || context_menu_actions[title]) {
      return false;
    }

    context_menu_items.push(title);
    context_menu_actions[title] = {
      module : module,
      action : action
    };
  };

  exposed_context_menu_remove = function(src, title) {
    var index = context_menu_items.indexOf(title);
    if (index === -1) {
      return false;
    }

    context_menu_items.splice(index, 1);
    delete context_menu_actions[title];
  };
  ...
  core_context_menu_handler = function(evt) {
    if(evt.src === 'mod_strip') {
      switch(evt.item) {
        ...
        default:
          if (context_menu_actions[evt.item]) {
            breach.module(context_menu_actions[evt.item].module).call(context_menu_actions[evt.item].action, {
              evt : evt,
              params : my.context_menu_params
            });
          }
      }
    }
  ...
  exposed_context_menu_builder = function(src, args, cb_) {
    ...
    var items = context_menu_items.slice();
  ...
  init = function(cb_) {
    async.series([
      function(cb_) {
        ...
        breach.expose('context_menu_add', exposed_context_menu_add);
        breach.expose('context_menu_remove', exposed_context_menu_remove);
      ...
      function(cb_) {
        ...
        breach.module('core').on('tabs:removed', core_tabs_removed_handler);
        return cb_();
      },
      function(cb_) {
